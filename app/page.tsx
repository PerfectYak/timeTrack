...
function AppShell() {
  ...
  return <div className="shell"><aside><div className="brand"><div className="logo">◷</div><div><strong>{settings.language === "hu" ? "Időkeret" : "Timeframe"}</strong><span>{t("worktime")}</span></div></div><nav>{nav.map((item) => <button key={item.id} className={tab === item.id ? "active" : ""} onClick={() => setTab(item.id)}><i>{item.icon}</i>{t(item.label)}</button>)}</nav><div className="storage"><span>●</span><div><strong>{t("localStorage")}</strong><small>{t("localStorageNote")}</small></div></div><div className="app-version" aria-label={`Version ${APP_VERSION_LABEL}`}>{APP_VERSION_LABEL}</div></aside>
